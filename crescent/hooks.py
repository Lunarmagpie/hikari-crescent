...
def add_hooks(
    ...
    if isinstance(obj.metadata, AppCommandMeta):
        obj.metadata.add_hooks(command_hooks, prepend=True, after=False)
        obj.metadata.add_hooks(command_after_hooks, prepend=True, after=True)
    elif isinstance(metadata := obj.metadata, EventMeta):
        metadata.add_hooks(event_hooks, prepend=True, after=False)
        metadata.add_hooks(event_after_hooks, prepend=True, after=True)
